...
The startup app source is the default trusted WebView named `main`. Simple apps can ignore this and keep using one page. Advanced apps can resize `main` like any other WebView and place isolated child WebViews beneath or above it.
...
System WebView stacks are implemented on macOS and Linux with layers, best-effort transparency, bridge-enabled trusted child WebViews, and page zoom. Windows support uses WebView2 with layers and zoom; transparency and bridge-enabled child WebViews are best effort as WebView2 support matures. macOS CEF supports layered child WebViews and zoom. Linux CEF explicitly fails WebView creation until real native CEF embedding is wired in.
